# dashboard-67f1cb3b 再開手順

このドキュメントは、Claude Code セッションを終了したり別の Mac に切り替えた後で、
`dashboard-67f1cb3b` の開発・運用作業をスムーズに再開するためのチェックリスト。
...
| 項目 | 値 |
|---|---|
| 公開 URL | `https://app.instyle.group/dashboard-67f1cb3b/` |
| GitHub | `https://github.com/sasaki-ta-instyle/dashboard-67f1cb3b`（Private 想定） |
| ConoHa デプロイ先 | `/var/www/app/dashboard-67f1cb3b/` |
| 共有 env | `/var/www/_shared/apps/app-dashboard-67f1cb3b.env`（chmod 600） |
| PM2 名 | `app-dashboard-67f1cb3b` |
| ポート | `3004` |
| Healthcheck | `/` |
| USE_DB | `false` |
...
```bash
mkdir -p ~/Workspace
gh repo clone sasaki-ta-instyle/dashboard-67f1cb3b ~/Workspace/dashboard-67f1cb3b
cd ~/Workspace/dashboard-67f1cb3b
```
...
`~/Workspace/dashboard-67f1cb3b/.env.local` に必要なキーを揃える（**git に入れない**）。
本番 `app-dashboard-67f1cb3b.env` と概ね同じ値で動く。
...
pnpm dev
# → http://localhost:3004/dashboard-67f1cb3b/ にアクセス
```
...
| 種類 | 場所 | 引き継ぎ方法 |
|---|---|---|
| ソースコード | GitHub `sasaki-ta-instyle/dashboard-67f1cb3b` | `git clone` |
| Claude Code 設定（memory / agents / skills） | `instyle-claude-sasaki` リポジトリ | bootstrap で同期 |
| 本番 Web プロセス（PM2） | ConoHa `app-dashboard-67f1cb3b` | 触らない、`deploy-prod.yml` で更新 |
| 本番 env（API キー類） | ConoHa `/var/www/_shared/apps/app-dashboard-67f1cb3b.env` | サーバ側永続、`ssh conoha-deploy` で参照可 |
| 本番 DB（SQLite の場合） | ConoHa `/var/www/app/dashboard-67f1cb3b/data/...` | サーバ側永続 |
...
```bash
gh workflow run deploy-prod.yml --ref main -R sasaki-ta-instyle/dashboard-67f1cb3b
gh run watch -R sasaki-ta-instyle/dashboard-67f1cb3b
```
...
```bash
ssh conoha-deploy '
sed -i "s|^KEY_NAME=.*|KEY_NAME=new_value|" /var/www/_shared/apps/app-dashboard-67f1cb3b.env
cd /var/www/app/dashboard-67f1cb3b/current && pm2 startOrReload ecosystem.config.cjs --update-env
'
```
...
```bash
ssh conoha-deploy 'pm2 logs app-dashboard-67f1cb3b --nostream --lines 50 --raw'
```

### 本番 PM2 再起動

```bash
ssh conoha-deploy 'pm2 restart app-dashboard-67f1cb3b --update-env'
```
